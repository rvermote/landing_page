...
const generateEmailContent = (data: EmailData) => {
    ...
    const htmlData = Object.entries(data).reduce((str,[key, val]:string[]) => 
        str += `<h1 className="">${CONTACT_MESSAGE_FIELDS[key]}</h1><p>${val || "anonymous"}</p>`, "")

    return {
        subject: `New Message from ` + (data.name || "anonymous"),
        text: stringData,
...
export async function POST(request: Request) {
    ...
    if (!data.email) return NextResponse.json({message: "Email is required"}, {status: 400})
    try {
